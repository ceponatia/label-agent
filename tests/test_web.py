...
def test_no_access_token_leaves_dashboard_open(tmp_path):
    config = Config(data_dir=str(tmp_path / "data"))
    db = Database(config.db_path)
    db.init()
...
def test_access_token_rejects_missing_or_wrong_credentials(tmp_path):
    config = Config(data_dir=str(tmp_path / "data"), web_access_token="s3cret")
    db = Database(config.db_path)
    db.init()
    client = TestClient(create_app(db, config, StubController()))
    ...
    assert no_auth.headers["www-authenticate"] == 'Basic realm="Label Agent"'

    wrong_auth = client.get("/", auth=("ignored", "wrong"))
    assert wrong_auth.status_code == 401

    db.close()


def test_access_token_accepts_correct_credentials(tmp_path):
    config = Config(data_dir=str(tmp_path / "data"), web_access_token="s3cret")
    db = Database(config.db_path)
    db.init()
    client = TestClient(create_app(db, config, StubController()))
...
def test_access_token_gates_api_and_static_routes(tmp_path):
    """Pins that middleware/mount ordering doesn't accidentally exempt a
    route that leaks label data or a static asset."""
    config = Config(data_dir=str(tmp_path / "data"), web_access_token="s3cret")
    db = Database(config.db_path)
    db.init()
    client = TestClient(create_app(db, config, StubController()))

    api_no_auth = client.get("/api/labels")
    assert api_no_auth.status_code == 401

    static_no_auth = client.get("/static/style.css")
    assert static_no_auth.status_code == 401

    api_auth = client.get("/api/labels", auth=("anything", "s3cret"))
    assert api_auth.status_code == 200

    db.close()


def test_access_token_rejects_non_ascii_password_without_crashing(tmp_path):
    config = Config(data_dir=str(tmp_path / "data"), web_access_token="s3cret")
    db = Database(config.db_path)
    db.init()
    client = TestClient(create_app(db, config, StubController()))

    response = client.get("/", auth=("anything", "café"))

    assert response.status_code == 401
    db.close()


def test_access_token_rejects_malformed_authorization_header(tmp_path):
    config = Config(data_dir=str(tmp_path / "data"), web_access_token="s3cret")
    db = Database(config.db_path)
    db.init()
    client = TestClient(create_app(db, config, StubController()))
