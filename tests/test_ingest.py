...
def test_ingested_rows_have_the_expected_fields(db, config):
    ...
    assert vinted.order_ref == "58412906634"
...
def test_forwarded_copies_with_new_tracking_are_new_labels(db, config):
    ...
    items.append(
        (
            ...
            raw("poshmark-forwarded").replace(
                POSHMARK_TRACKING.encode(), b"9581357024681012345790"
            ),
        )
    )
    items.append(
        (
            ...
            raw("vinted-forwarded").replace(
                VINTED_TRACKING.encode(), b"9581357024681012345791"
            ),
        )
    )
    ...
    assert db.get_label(3).tracking_number == "9581357024681012345790"
