...
# from the fixture via raster_label_bbox: Rect(35.52, 89.76, 324.0, 521.76).
VINTED_LABEL_RECT = (35.52, 89.76, 324.0, 521.76)
...
# whole letter page covers ~6%, well under CROP_INK_FLOOR. Unlike a real
# scanned label, the synthetic barcode is drawn clean enough that it still
# decodes even at that whole-page scale (see
# test_whole_page_fallback_would_fail_the_crop_assertions below), so
# ink coverage -- not barcode decodability -- is what discriminates a real
# crop from this fallback for this fixture.
CHECK_DPI = 150
CROP_INK_FLOOR = 0.12
...
def test_vinted_bounding_box_matches_the_printed_label():
    ...
    # abs=0.6 pt is well under one 150-dpi pixel (0.48pt) of true precision
    # but leaves headroom for antialiasing differences across MuPDF builds/
    # platforms, rather than pinning to the sub-pixel value measured here.
    assert tuple(bbox) == pytest.approx(VINTED_LABEL_RECT, abs=0.6)
...
def test_vinted_fixture_is_cropped_to_the_label(tmp_path):
    ...
    assert result.ok
    # This fixture is a single embedded raster image with no vector drawing
    # objects (see tests/fixtures/make_label_fixtures.py), so vector_label_bbox
    # cannot fire and the pipeline must take the raster-crop branch -- the one
    # real Vinted PDFs actually take. Pinned exactly rather than "either crop
    # method" so a future regeneration that accidentally adds vector content
    # can't silently drop coverage of that branch.
    assert result.method == "raster-crop"
...
def test_whole_page_fallback_would_fail_the_crop_assertions(tmp_path):
    """Guards the ink-coverage threshold above: an uncropped vinted page stays
    under CROP_INK_FLOOR even though its barcode still decodes.

    A real scanned Vinted label's barcode degrades below readability at this
    scale; this synthetic one is drawn clean enough that it does not, which
    is why ink coverage -- not decodability -- is the signal asserted here.
    That fact is asserted explicitly (not just claimed in this docstring) so
    a future fixture regeneration that changes it doesn't silently invalidate
    this explanation.
    """
