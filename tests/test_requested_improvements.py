...
def test_real_poshmark_fixture_saves_buyer_name(tmp_path):
    ...
    try:
        ...
        assert db.get_label(1).buyer_name == "Priya Nolan"
    finally:
        db.close()
    ...
    try:
        label = db.insert_label(
            Label(
                ...
                buyer_name="Taylor Brooks",
                gmail_message_id="buyer-test",
                status=LabelStatus.PRINTED,
            )
        )
        assert db.get_label(label.id).buyer_name == "Taylor Brooks"
    finally:
        db.close()
