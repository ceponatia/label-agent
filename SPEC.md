...
**Status:** Historical design document, accompanies [SCOPE.md](SCOPE.md). The
v1 described here has been built; see [README.md](README.md) for current
usage. Text marked *(assumption)* records a decision made along the way, not
an open question.

## 1. System overview

```
                ┌────────────────── host machine (seller's laptop → home server later) ──────────┐
                │                                                                                │
 ...
 (poll)         │                                                                (4×6 rear tray) │
                ...
                └───────────────────────────────────│────────────────────────────────────────────┘
                                                    ▼
                              Seller's phone / tablet / laptop browser
```
...
Platform emails are addressed directly to the seller's monitored Gmail account
(e.g. `seller@example.com`), so the agent authenticates only to that account.

**Auth: IMAP + Google app password** (requires 2-Step Verification on the
...
with a `label-agent/processed` Gmail label and archive them (removing the
`\Inbox` label), and `X-GM-RAW` for Gmail-syntax searches. The mailbox is
opened read-only except for those label writes on a message once it's been
safely handled; nothing is ever marked read or deleted, and archiving only
moves a message out of Inbox into All Mail, where it stays intact and
searchable.
...
Tested example device: **Canon PIXMA TS9521Ta on WiFi, 4×6 stock in the rear
tray**. The TS9500 series supports AirPrint (driverless IPP), which CUPS —
built into macOS — speaks natively, so no vendor driver is needed; the printer
is addressed by its network name from any host on the LAN. Any AirPrint (or,
on Windows, any normally-installed) printer works the same way.
...
Binds to `127.0.0.1:8080` on the host by default, so a fresh install is
reachable only from the machine it runs on. Reaching it from a phone or
another device on the LAN requires explicitly setting `web_host` /
`LABELAGENT_WEB_HOST` to `0.0.0.0` or a specific LAN IP — there is still no
account system, so once widened the dashboard is reachable by anything on
that network. An optional shared-token gate (`web_access_token` /
`LABELAGENT_WEB_ACCESS_TOKEN`, checked as an HTTP Basic password) can be set
alongside LAN access to require a shared secret before serving any page; it
is plain HTTP Basic auth, not encryption, so it keeps out casual devices but
not anyone who can already sniff the LAN. Server-rendered, responsive layout
(single column on phone). Optional PWA manifest so it can be added to the
iPhone home screen.
...
- Gmail credentials: a Google **app password** for the seller's Gmail account
  in `.env` (one-time setup: enable 2-Step Verification, generate app
  password). No OAuth flow, nothing expires.
...
- **Host 1 (now): the seller's macOS laptop.** launchd agent (`KeepAlive`),
  CUPS built in, WiFi printer added by network name. Laptop sleep is fine: on
  wake, the next poll catches up on anything missed; nothing prints while it's
  closed. Optional Dockerfile + compose as the OS-independent path.
...
email that passes classification exists as a row the seller can see and act on.

## 11. Testing

- **Golden files:** the six example emails and two example labels in this repo
  become fixture tests — full pipeline offline (mock Gmail, mock `lp`),
  asserting output page size, orientation, and successful barcode detection
  (decode the Code128 with `pyzbar` as an objective crop check).
...
- **P1 done:** `python -m labelagent process tests/fixtures/*.pdf` produces two valid
...
- **P3 done:** the seller pauses, resumes, reprints, and reads yesterday's
  history from their phone.
