...
class Config:
    ...
    # Local-only by default so a fresh install isn't reachable from the
    # network until someone deliberately opts in (set to "0.0.0.0" or a
    # LAN IP). See README "Privacy & security" before widening this.
    web_host: str = "127.0.0.1"
    web_port: int = 8080
    # When set, the web app requires this value as an HTTP Basic password
    # (any username) before serving anything. Recommended whenever web_host
    # is widened beyond 127.0.0.1.
...
FIELDS: dict[str, tuple[type, str]] = {
    ...
    "web_access_token": (str, "LABELAGENT_WEB_ACCESS_TOKEN"),
