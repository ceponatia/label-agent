"""FastAPI control panel for the label agent (binds to 127.0.0.1 by default
per SPEC §7; LAN access and an optional shared-token gate are opt-in)."""
...
class _AccessTokenMiddleware(BaseHTTPMiddleware):
    """Gate every request behind HTTP Basic auth using a single shared token.

    The username is ignored; the password must match ``web_access_token``.
    This is a lightweight opt-in for LAN/phone use, not a real user model —
    there is one shared secret and no sessions or accounts.
    """
    ...
    async def dispatch(self, request: Request, call_next):
        if not _basic_auth_ok(request.headers.get("authorization"), self._token):
            return Response(
                content="Authentication required.",
                media_type="text/plain",
                status_code=401,
                headers={"WWW-Authenticate": 'Basic realm="Label Agent"'},
            )
...
def _basic_auth_ok(header: str | None, token: str) -> bool:
    ...
    # non-ASCII str input, which would otherwise turn a bad password into an
...
def create_app(db: Database, config: Config, controller: AgentController) -> FastAPI:
    ...
    if config.web_access_token:
        app.add_middleware(_AccessTokenMiddleware, token=config.web_access_token)
